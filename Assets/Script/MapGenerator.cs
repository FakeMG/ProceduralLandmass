...
namespace NoiseMap {
    public class MapGenerator : MonoBehaviour {
        ...
        // factor of 241-1 is beatiful (1,2,4,6,8,12)
        // suitable for levelOfDetail in meshGenerator
        ...
        private MapData GenerateMapData(Vector2 center) {
            float[,] noiseMap = Noise.GenerateNoiseMap(MAP_CHUNK_SIZE + 2, MAP_CHUNK_SIZE + 2, seed, noiseScale, octaves, persistance, lacunarity, center + offset, normalizeMode);

            Color[] colourMap = new Color[MAP_CHUNK_SIZE * MAP_CHUNK_SIZE];
            for (int y = 1; y <= MAP_CHUNK_SIZE; y++) {
                for (int x = 1; x <= MAP_CHUNK_SIZE; x++) {
                    ...
                    for (int i = 0; i < regions.Length; i++) {
                        if (currentHeight >= regions[i].height) {
                            colourMap[(y - 1) * MAP_CHUNK_SIZE + (x - 1)] = regions[i].colour;
